...
function getWinnersByYear(arr, cb1, cb2) {
    return cb1(arr, getFinals).map(item => {
        return `In ${item}, ${cb2(arr, getFinals)[cb1(arr, getFinals).indexOf(item)]} won the world cup!`;
    })
}
...
function getAverageGoals(cb) {
    let ave;
    let sum = cb.reduce((acc, item) => acc + item['Home Team Goals'], 0) + cb.reduce((acc, item) => acc + item['Away Team Goals'], 0);
    return ave = (sum/cb.length).toFixed(2);
}
